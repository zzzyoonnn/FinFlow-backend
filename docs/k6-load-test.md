...
k6는 20 VU가 30초 동안 완료된 동일 키를 반복 호출하는 조건으로 실행했다. 저장된 실제 부하 테스트 결과는 다음과 같다.
...
이 결과에서 Redis 캐시 경로의 평균 응답시간은 약 77.45%, p95는 약 73.16% 감소했고 처리량은 약 4.26배 증가했다. k6에서는 평균만 보지 않고 동일한 조건에서 측정한 p95, p99, 실패율, 처리량 차이를 함께 확인한다.
